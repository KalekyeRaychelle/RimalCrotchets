...
const CatalogForm = () => {
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      const response=await fetch("https://localhost:7700/api/Catalog",{
        method:"POST",
  ...
  return (
    <div className='CatalogForm'>
      <Link to='/Catalog'><FaAngleLeft /></Link>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input
         ...
         id='productName' placeholder='Cherry Patterned Sweater'
         value={catalogData.productName} onChange={handleChange} required
         />
       <div className='categoryForm'>
        ...
        <label>Price</label>
        <input type='number' value={catalogData.Price} onChange={handleChange} required
         name="Price" id="Price" placeholder='ENTER NUMBER ONLY'/>
        <label>Product Image</label>
        <input type='file' name="productImagePath" id="productImagePath"
        placeholder='ADD PRODUCT IMAGE'
        value={catalogData.productImagePath} onChange={handleChange} required />
